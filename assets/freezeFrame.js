...
const freezeframe = (function ($) {
  let images, options, is_touch_device, default_state;
  ...
  freezeframe.prototype.setup = function (_selector) {
    ...
    imagesLoaded(setup_required).on("progress", (instance, image) => {
  ...
  freezeframe.prototype.attach = function (_selector) {
    ...
    filter.call(ff, _selector, ff.images).each(function (e) {
      const $image = $(this);
      const $canvas = $(this).siblings("canvas");
      ...
      if (
        ...
        $image.click(() => {
          (function () {
            const clicked = $canvas.hasClass("ff-canvas-active");
...
$.fn.freezeframe = function (_options) {
  ...
  const ff = new freezeframe(_options);
